...
@app.route('/track_date', methods = ['POST'])
def track_date():
...
def main(track, date):
    ...
    for race in races:
        ...
        headers = "Horse, Rating, Win rate, Avg \n"
        f.write(headers)
        ...
        for horse in array:
            ...
            stats = (horse[1] + horse[2]) / 2
            if stats  >= best_stats[1]:
                best_stats = [horse[0], stats]
            line += (str(stats))

            f.write(line + '\n')

        f.write('\n')
        f.write('Winner: ' + best_stats[0] + ", \n" + "Points: " + str(best_stats[1]))
        f.write('\n')
        f.write('\n')
        f.write('\n')
